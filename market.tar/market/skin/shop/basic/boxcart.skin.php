...
<aside id="sbsk">
    ...
    foreach($cart_datas as $row)
    {
        ...
        echo '<a href="'.G5_SHOP_URL.'/cart.php" class="prd_cnt">'.$it_name.'</a>';
        echo '<span class="prd_cost">';
		echo number_format($row['ct_price']).PHP_EOL;
        echo '원</span>'.PHP_EOL;
		echo '</div>';
		echo '<button class="cart_del" type="button" data-it_id="'.$row['it_id'].'"><i class="far fa-trash-alt"></i><span class="sound_only">삭제</span></button>'.PHP_EOL;
        echo '</li>';
    ...
    <?php if($i){ ?><button  type="submit" class="btn02 btn_buy">바로구매</button></div><?php } ?>
    <a href="<?php echo G5_SHOP_URL; ?>/cart.php" class="btn01 go_cart">전체보기</a>
    </form>
</aside>
<script>
jQuery(function ($) {
